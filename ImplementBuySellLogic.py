...
from BuySellSignals import add_buy_signals, add_sell_signals, add_macd_crossover_state, add_macd_crossover_state_for_sell  # Import the functions
...
def load_tickers(excel_file, worksheet):
    try:
        ...
        df = df[['Ticker', 'TBUSpreadPercentage','Avg High Low','RSI_Buy_Threshold','RSI_Sell_Threshold','Gap%', 'MarketBodyRatio']].dropna()
        ...
        ticker_spread_dict = dict(zip(df['Ticker'], zip(df['TBUSpreadPercentage'],df['Avg High Low'],df['Gap%'],df['MarketBodyRatio'])))
...
def load_data(ticker_spread_dict, save_dir="ALPACA_DATA", macd_thresholds=None):
    data = {}
    signal_log = []  # To store signal information
    for ticker, (spread_percentage, Avg_HL_Sprd, gap_pct, MarketBodyRatio) in ticker_spread_dict.items():
        ...
        try:
            ...
            df, last_buy_price = add_buy_signals(df, ticker_name=ticker, fib_levels=fib_levels, gap_pct = gap_pct, MarketBodyRatio = MarketBodyRatio, 
            ...
            file_path = "/Users/jimutmukhopadhyay/Dummy Trading/IntraDay Trading/BuySellSignals.xlsx"
...
def calculate_mfi(df, window=14):
    ...
    sum_pos = pos_mf.rolling(window, min_periods=window).sum()
    sum_neg = neg_mf.rolling(window, min_periods=window).sum()

    # Avoid divide-by-zero
    mfr = sum_pos / sum_neg.replace(0, np.nan)
...
def main():
    ...
    #excel_file = "/Users/jimutmukhopadhyay/Dummy Trading/IntraDay Trading/filtered_tickers.xlsx"
    excel_file = "/Users/jimutmukhopadhyay/Dummy Trading/IntraDay Trading/filtered_tickers_All.xlsx"
    ...
    ticker_spread_dict = {k: ticker_spread_dict[k] for k in list(ticker_spread_dict.keys())[:300]}
    ...
    for ticker in tickers:
        ...
        if not macd_data.empty:
            macd_thresholds_dict[ticker] = {
                ...
                "gap_pct":macd_data["Gap%"].values[0],
                "MarketBodyRatio":macd_data["MarketBodyRatio"].values[0]
            }
    ...
    html.Div([
        ...
        html.Label("Select Date:", style={"margin-right": "10px"}),
        dcc.DatePickerSingle(
            id="date-picker",
            date=(datetime.today() - timedelta(days=1)).strftime('%Y-%m-%d'),
            min_date_allowed=(datetime.today() - timedelta(days=90)).strftime('%Y-%m-%d'),
    ...
    def update_charts(selected_ticker, selected_date, selected_indicator):
        ...
        # Convert selected date to datetime range for the trading day
        try:
            import pytz
            # start_datetime = datetime.strptime(selected_date, "%Y-%m-%d") + timedelta(hours=9, minutes=30)
            # end_datetime = datetime.strptime(selected_date, "%Y-%m-%d") + timedelta(hours=16)
            # Ensure datetime values are timezone-aware
            ny_tz = pytz.timezone('America/New_York')
            start_datetime = ny_tz.localize(datetime.strptime(selected_date, "%Y-%m-%d") + timedelta(hours=9, minutes=30))
            end_datetime = ny_tz.localize(datetime.strptime(selected_date, "%Y-%m-%d") + timedelta(hours=16))
        except Exception as e:
            print(f"Error parsing selected_date '{selected_date}': {e}")
            return {}, {}, {}, {}

        # Filter the DataFrame for the selected date
        df_filtered = df[(df['Datetime'] >= start_datetime) & (df['Datetime'] <= end_datetime)]
        if df_filtered.empty:
            print(f"Filtered DataFrame is empty for {selected_ticker} on {selected_date}.")
        #return {}, {}, {}, {}
        ...
        print("DF Filtered ticker", ticker,"\n",df_filtered.head(),"\n")
        # Debug: Log Buy and Sell signal counts
        ...
        # Add Dynamic Support levels
        # candlestick_fig.add_trace(
        #     go.Scatter(
        #         x=df_filtered['Datetime'],
        #         y=df_filtered['Dynamic_Support'],
        #         mode='lines',
        ...
        # # Add Dynamic Resistance levels
        # candlestick_fig.add_trace(
        #     go.Scatter(
        #         x=df_filtered['Datetime'],
        #         y=df_filtered['Dynamic_Resistance'],
        #         mode='lines',
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=12) as executor:
        ...
        future_reqAccDetailsAlpaca = executor.submit(run_script, "/Users/jimutmukhopadhyay/Dummy Trading/AlpacaTrading/ReqExeAcntPosAlapaca.py")
        ...
        futures = [future_reqAccDetails, future_main, future_reqOpenOrders]

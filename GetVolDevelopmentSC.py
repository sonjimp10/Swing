...
# ---------------- CONFIG ----------------
INTRADAY_DIR = Path("ALPACA_SC_DATA")   # folder with 5-min CSVs (CET/Europe/Amsterdam timestamps)
TICKERS_CSV  = Path("SmallCap.csv")  # has columns: Ticker, FloatShares (case-insensitive OK)
...
def main():
    ...
    long_outfile = Path(f"morning_volume_cum_long_SC_{et_date.date()}.csv")
    ...
    wide_outfile = Path(f"morning_volume_cum_wide_SC_{et_date.date()}.csv")

...
- Reverse DCF (implied WACC); WACC policy switch remains.
- OUTPUT: only "Summary" and "PerYear" sheets.
- Summary now also includes ValuePerShare_Sheet, ValuePerShare_Average, ValuePerShare_Implied.
...
WACC_POLICY = "sheet"  # change to "average" to blend sheet & implied WACC
...
def main():
    ...
    for _, r in w.iterrows():
        ...
        # Decide WACC in use per WACC_POLICY (kept for continuity)
        ...
        # EV/Equity/Per-share using chosen WACC (existing output)
        ...
        # NEW: compute VPS under all three policies (sheet, implied, average)
        vps_sheet = np.nan
        vps_implied = np.nan
        vps_average = np.nan

        if is_finite(wacc_sheet):
            ev_s = ev_from_cashflows(fcff_list, wacc_sheet, tg)
            eq_s = ev_s - (netdebt if is_finite(netdebt) else 0.0)
            vps_sheet = eq_s / shares if (is_finite(shares) and shares > 0) else np.nan

        if is_finite(implied_wacc):
            ev_i = ev_from_cashflows(fcff_list, implied_wacc, tg)
            eq_i = ev_i - (netdebt if is_finite(netdebt) else 0.0)
            vps_implied = eq_i / shares if (is_finite(shares) and shares > 0) else np.nan

        if is_finite(wacc_sheet) and is_finite(implied_wacc):
            w_avg = 0.5 * (wacc_sheet + implied_wacc)
            ev_a = ev_from_cashflows(fcff_list, w_avg, tg)
            eq_a = ev_a - (netdebt if is_finite(netdebt) else 0.0)
            vps_average = eq_a / shares if (is_finite(shares) and shares > 0) else np.nan
        ...
        summary_rows.append({
            ...
            "EV_from_DCF": EV,
            "EquityValue": equity,
            "ValuePerShare": vps,  # based on WACC_Used (for continuity)
            "ValuePerShare_Sheet": vps_sheet,       # NEW
            "ValuePerShare_Average": vps_average,   # NEW
            "ValuePerShare_Implied": vps_implied,   # NEW
    ...
    sum_cols = [
        ...
        "ValuePerShare_Sheet","ValuePerShare_Average","ValuePerShare_Implied",  # NEW
    ...
    with pd.ExcelWriter(OUTPUT_XLSX, engine="openpyxl") as w:
        ...
        # NOTE: sensitivity sheets removed per request.
